...
export default function AddNewTaskButton() {
    ...
    return (
        <div className="fixed bottom-8 right-10 z-50 flex flex-col items-end">
                ...
                {showCreateOptions.list && (
                    <>
                        <button
                            ...
                            onClick={() => setShowCreateOptions((prev) => ({...prev, list: false, project: true}))}
                        >
                            Создать проект
                        </button>
                        <button
                            className="text-white rounded-full shadow-xl flex justify-center items-center bg-blue-500 hover:bg-blue-600 cursor-pointer py-2 px-4 mb-3"
                            onClick={() => setShowCreateOptions((prev) => ({...prev, list: false, task: true}))}
                        >
                            Создать задачу
                        </button>
                    </>
            ...
            {(!showCreateOptions.task && !showCreateOptions.project) &&<button
                className={clsx(
                    "size-12  text-white rounded-full shadow-xl flex justify-center items-center bg-blue-500 hover:bg-blue-600 cursor-pointer",
                    { "bg-gray-400/75 hover:bg-gray-400": showCreateOptions.list }
                )}
                onClick={() => setShowCreateOptions((prev) => ({...prev, list: !prev.list}))}
            >
                <AiOutlinePlus
                    className={clsx({
            ...
            {showCreateOptions.task &&
                createPortal(
                    <CreateNewTask onClose={() => setShowCreateOptions(prev => ({...prev, task: false}))} />,
                    document.body
                )}
            {showCreateOptions.project &&
                createPortal(
                    <CreateNewProject onClose={() => setShowCreateOptions(prev => ({...prev, project: false}))} />,
                    document.body
                )}
        </div>
    );
